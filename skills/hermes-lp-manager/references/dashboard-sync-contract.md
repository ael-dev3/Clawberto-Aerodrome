...
The LP control plane must prioritize LP uptime/profitability and post-state verification. The default path is a deterministic local watcher plus a 1-minute Hermes supervisor gated by `wakeAgent`; neither path should modify `src/positions.ts`, run tests/build, commit/push, pull/rebase, or wait for Pages. Those actions belong to a separate release workflow or the explicit `HERMES_DASHBOARD_SYNC=1` path.
