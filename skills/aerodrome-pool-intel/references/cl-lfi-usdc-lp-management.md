...
5. Prefer the split supervisor architecture: local deterministic watcher every 15s writes `runs/aerodrome-lp-supervisor/state.json`; Hermes runs only a 1-minute script-backed supervisor that stays silent when `wakeAgent=false`. Keep at least `HERMES_REBALANCE_COOLDOWN_SECONDS=600` so range drift does not continuously burn gas. Stake remediation and orphan cleanup bypass the churn cooldown only when explicitly guarded.
